...
const template = (string, object, customReplacers, context = null) => {
  if (context && object) {
    log({
      context,
      message: `Templating ${string} with object ${JSON.stringify(object)}`,
    })
  }
  let input = string.replace(/(\$[A-Z_]+)/g, (_, k) => {
    ...
    if (context) {
      log({
        context,
        message: `Replaced by regex k ${k} in string ${string}: it was resolved to ${result}`,
      })
    }
    return result
  })
